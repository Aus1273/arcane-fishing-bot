use fishing_core::{
    config::{macbook_profile, EnergyFailurePolicy},
    detection::EnergyReading,
    engine::{Action, Controller, Observation, Phase},
};
fn config() -> fishing_core::config::BotConfig {
...
fn feed_uses_capacity_and_counts_only_verified_restoration() {
    ...
    assert_eq!(c.feeds, 0);
    assert_eq!(c.phase, Phase::RestoringRod);
    let mut failed_restoration = c.clone();
    failed_restoration.action_failed("Rod shortcut failed");
    assert_eq!(failed_restoration.feeds, 0);
...
fn failed_ocr_or_input_never_counts_a_feed() {
    let mut config = config();
    config.fish_per_feed = 1;
    config.energy_monitoring_enabled = true;
    let mut c = Controller::new(config);
    caught(&mut c);
...
#[test]
fn optional_monitoring_is_off_by_default() {
    let mut config = config();
    config.fish_per_feed = 1;
    let mut c = Controller::new(config);
    caught(&mut c);
    assert_eq!(c.phase, Phase::Cooldown);
    assert!(!c.needs_energy());
}

#[test]
fn optional_ocr_failure_policy_never_issues_input() {
    for policy in [EnergyFailurePolicy::Continue, EnergyFailurePolicy::Pause] {
        let mut config = config();
        config.fish_per_feed = 1;
        config.energy_monitoring_enabled = true;
        config.energy_failure_policy = policy;
        let mut c = Controller::new(config);
        caught(&mut c);
        assert!(c.needs_energy());
        let mut o = obs(900);
        o.energy_error = Some("OCR unavailable".into());
        assert!(tick(&mut c, 900, o).is_empty());
        assert_eq!(
            c.phase,
            if policy == EnergyFailurePolicy::Continue {
                Phase::Cooldown
            } else {
                Phase::Paused
            }
...
#[test]
fn feeding_forces_monitoring_and_never_ignores_ocr_failure() {
    let mut config = config();
    config.fish_per_feed = 1;
    config.auto_feed_enabled = true;
    config.energy_capacity_feed_below = 500;
    assert!(!config.energy_monitoring_enabled);
    assert!(config.monitors_energy());
    let mut c = Controller::new(config);
    caught(&mut c);
    assert!(c.needs_energy());
    let mut o = obs(900);
...
#[test]
fn missing_ocr_value_obeys_optional_policy_at_deadline() {
    let mut config = config();
    config.fish_per_feed = 1;
    config.energy_monitoring_enabled = true;
    let mut c = Controller::new(config);
    caught(&mut c);
    assert!(tick(&mut c, 5811, obs(5811)).is_empty());
    assert_eq!(c.phase, Phase::Cooldown);
    assert_eq!(c.feeds, 0);
}

#[test]
fn capture_errors_are_never_treated_as_optional_ocr_failure() {
    let mut config = config();
    config.fish_per_feed = 1;
    config.energy_monitoring_enabled = true;
    let mut c = Controller::new(config);
    caught(&mut c);
    for time in [900, 1000, 1100] {
        let mut o = obs(time);
        o.error = Some("Screen capture unavailable".into());
        o.energy_error = Some("OCR unavailable".into());
        assert!(tick(&mut c, time, o).is_empty());
    }
    assert_eq!(c.phase, Phase::Paused);
}
#[test]
...
fn json_replays_match_expected_actions() {
        ...
        std::fs::read_dir(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/replays")).unwrap()
    {
        ...
        if entry.path().extension().is_some_and(|v| v == "json") {
            ...
            fishing_core::replay::run(scenario).unwrap();
        }
    }
}
