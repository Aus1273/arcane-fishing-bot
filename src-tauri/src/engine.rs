...
use crate::{config::BotConfig, detection::EnergyReading};
    ...
    pub fn step(
        ...
        if now > self.deadline && self.phase != Phase::Cooldown {
            match self.phase {
                ...
                Phase::CheckEnergy => {
                    self.pause("Energy could not be read; inspect the calibration")
                }
        ...
        match self.phase {
                    ...
                    && now.saturating_sub(self.entered) >= 100 =>
            {
                ...
                if self.fish_caught % u64::from(self.config.fish_per_feed) == 0 {
